...
def telebot_notify_sync(insti_id, message):
    object = findTelegramChatID(insti_id)
    if not object:
        return
    chat_id = object.chat_id
...
bot.message_handler(commands=['register'])
def register_telegram_chat_id(message):
    chat_id = message.chat.id
    insti_id =message.text.split()[1]
    existing_chat_id = findTelegramChatID(insti_id)
    if not existing_chat_id:
        sapien_instance = findSapien(insti_id)
        createTelegramChatID(sapien=sapien_instance, chat_id=chat_id)
        telebot_notify_sync(insti_id, "Registeration Successful")
        return
    telebot_notify_sync(insti_id, "You're already registered")
